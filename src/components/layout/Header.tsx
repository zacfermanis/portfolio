...
const Header: React.FC = () => {
  ...
  return (
    <header className="bg-sky-600 shadow-sm border-b sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-sky-600 border-b border-gray-200 shadow-lg z-50">
            <nav className="px-4 py-2 space-y-1">
              {navigationItems.map((item) => (
                <a
                  ...
                  href={item.href}
                  onClick={closeMenu}
                  className="block text-white hover:text-gray-100 hover:bg-sky-700 px-3 py-2 rounded-md text-base font-medium transition-colors"
                >
                  {item.label}
                </a>
              ))}
